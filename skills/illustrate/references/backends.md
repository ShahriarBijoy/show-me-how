...
Result is a JSON line. `ok:true` → continue to labels. `ok:false` with `promptFile` (manual) → tell the user: "Prompt saved to <promptFile>. Paste it into ChatGPT/Gemini, save the image as `<out>`, then re-run the same slash command (e.g. `/show-me-how:explain <topic>`); the saved shot is picked up and labelled." and continue building the doc with `![NN — pending](raw/NN.png)`. `ok:false` with `stderr` (codex failed) → show the last 5 lines of stderr and fall back to manual for this shot.
